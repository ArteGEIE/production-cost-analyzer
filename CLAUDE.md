...
| Déploiement | Un conteneur + PostgreSQL, plateforme au choix du consommateur (un `docker-compose.yml` d'évaluation fourni en exemple) | Auth OIDC pluggable + mode démo |
...
- **OS-3** ✅ : Containeriser l'application (Dockerfile, `docker-compose.yml` avec Postgres)
...
- **Aucune donnée privée dans le dépôt** : pas de noms d'hôtes, d'IP, d'identifiants de projet cloud, de vrais noms de producteurs ni de fichiers `documents/`. Ce dépôt ne déploie rien et n'impose aucune plateforme : il publie une image et documente ce dont le conteneur a besoin (`docs/deployment.md`). Tout ce qui est propre à une instance (overlays, `custom.js`, workflow de déploiement) vit ailleurs.
...
- **Secrets hors dépôt** : en production les secrets viennent d'un gestionnaire de secrets (gestionnaire de secrets de la plateforme) ou du `.env` du serveur, jamais du dépôt
