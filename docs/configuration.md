...
Migrations: `npm run db:migrate` locally, `node scripts/migrate.mjs` inside the image (the
Compose file runs it before the app starts).
...
Add it to `public/` before building your image, or copy it into an image derived
from the published one (see `docs/deployment.md`). In development, restart
`npm run dev` after adding or removing it. The file is served like any static asset, so it must
not contain secrets.
