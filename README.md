...
- **Docker Compose** on a single host: [`docker-compose.prod.yml`](docker-compose.prod.yml)
  with the published image `ghcr.io/artegeie/production-cost-analyzer`
- **Kubernetes**: Helm chart in [`.infrastructure/helm/app`](.infrastructure/) with example
  overlays, optional External Secrets and a migration hook Job

Details in [`docs/deployment.md`](docs/deployment.md), including how to run your own
instance from a private deployment repository.
